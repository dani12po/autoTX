...
try:
    import importlib
    _mod = importlib.import_module("danixyz")
    danixyz = getattr(_mod, "danixyz")
    if not callable(danixyz):
        raise AttributeError("danixyz callable not found")
...
def display_header():
    ...
    print(Style.BRIGHT + Fore.CYAN + ascii_banner)
    print(Style.BRIGHT + Fore.CYAN + border + "\n")
    # Mempertahankan header lama agar tidak menghapus apapun
    print(Style.BRIGHT + Fore.CYAN + "======================================")
    print(Style.BRIGHT + Fore.CYAN + " AUTO TX MONAD BY DANI XYZ        ")
    print(Style.BRIGHT + Fore.CYAN + "====================================\n")

# Initialize danixyz if available (falls back to no-op if module missing)
danixyz()
